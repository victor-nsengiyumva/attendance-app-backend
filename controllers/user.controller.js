...
exports.signUp = async (req, res, next) => {
    ...
    if (data) {
        ...
        let loc = await db.location.findOne({ where: { name: { [Op.eq]: locationName } } });

        if(!loc){
            res.send("that location doesnt exist")
        }
...
exports.login = async (req, res, next) => {
    ...
    if (PF) {
        ...
        try {
            data = await User.findOne({ where: { PF: { [Op.eq]: PF } } ,include:db.location }) // this include at the end includes the location information from the foreign table
            // the rough sql equivalent for this command is  [ SELECT * FROM users WHERE PF = 'value';]
            //console.log(data.PF, data.id); /// You can actually access the properties of the returned data directly


            if (data) {
                console.log(data.PF, data.id);
                // Generate a token
                ...
                console.log("i am not working out")
